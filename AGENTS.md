...
Project rules, commands and architecture are in `CLAUDE.md` — read it first.

This project also ships Rayfin agent context.
...
If MCP is unavailable, run `rayfin docs ...` (or `npx -y @microsoft/rayfin-cli docs ...`) from the project root so the CLI reads this project's `node_modules`.
